...
object WasteSource {
  ...
  def apply(location:(Int, Int), orchestrator: ActorRef): Behavior[SourceCommands]
  = source(0, location, 0, orchestrator, 1)
      ...
      (context, message) => {
        message match {
          ...
          case GetGarbage(maxAmount, garbageCollector) =>
            context.log.debug("Garbage received")
          ...
          case ProduceGarbage(amount) => source(garbage + amount, location, score, orchestrator, sourceId)
        }
      }
    )
}
